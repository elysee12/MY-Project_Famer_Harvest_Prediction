...
import { 
  ...
  LuPencilLine, LuKey, LuGlobe, LuInfo, LuLogOut, LuChevronRight,
  LuShield, LuCalendar, LuEye, LuEyeOff
} from 'react-icons/lu';

export default function OfficerProfile({ user, onLogout, onBack, lang, setLang }) {
  const t = T[lang];
  ...
  const initials = (user.name || user.full_name || 'O').split(' ').map(n => n[0]).join('').toUpperCase();
  
  const isAdmin = user.role === 'admin' || user.role === 'district';
  const isSector = user.role === 'sector' || user.role === 'officer';

  const roleLabel = isAdmin 
    ? (lang === 'en' ? 'System Admin' : 'Umuyobozi wa Sisitemu')
    : (lang === 'en' ? 'Sector Officer' : 'Umukozi wa Segiteri');

  const handleUpdateProfile = async () => {
    if (!name.trim()) {
      return setStatus({ type: 'err', msg: lang === 'en' ? 'Name is required' : 'Izina rirakenewe' });
    }
    
    if (!email.trim() || !email.includes('@')) {
      return setStatus({ type: 'err', msg: lang === 'en' ? 'Valid email is required' : 'Imeri ifite uburyo bukwiye irakenewe' });
    }
    ...
    try {
      const res = await fetch(`${API_BASE}/api/update-officer-profile`, {
        method: 'POST',
      ...
      if (data.success) {
        setStatus({ type: 'ok', msg: lang === 'en' ? '✓ Profile updated successfully!' : '✓ Umwirondoro wahinduwe neza!' });
        // Update user object
        user.name = name;
        user.full_name = name;
        user.email = email;
        ...
        setStatus({ type: 'err', msg: data.error || (lang === 'en' ? 'Update failed' : 'Guhindura byanze') });
      }
    } catch (e) {
      setStatus({ type: 'err', msg: lang === 'en' ? 'Network error' : 'Ikosa ryo kuri network' });
    }
  ...
  const handleChangePassword = async () => {
    if (!currentPassword || !newPassword || !confirmPassword) {
      return setStatus({ type: 'err', msg: lang === 'en' ? 'All fields are required' : 'Byose birakenewe' });
    }
    
    if (newPassword !== confirmPassword) {
      return setStatus({ type: 'err', msg: lang === 'en' ? 'Passwords do not match' : 'Ibanga ntabwo rihuye' });
    }
    
    if (newPassword.length < 6) {
      return setStatus({ type: 'err', msg: lang === 'en' ? 'Password must be at least 6 characters' : 'Ibanga rigomba kuba nibura 6' });
    }
    ...
    try {
      const res = await fetch(`${API_BASE}/api/change-password`, {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          user_id: user.id || user.officer_id,
      ...
      if (data.success) {
        setStatus({ type: 'ok', msg: lang === 'en' ? '✓ Password changed successfully!' : '✓ Ijambo ryibanga ryahinduwe neza!' });
        setCurrentPassword('');
        setNewPassword('');
        setConfirmPassword('');
        setTimeout(() => setMode('view'), 2000);
      } else {
        setStatus({ type: 'err', msg: data.error || (lang === 'en' ? 'Password change failed' : 'Guhindura ijambo ryanze') });
      }
    } catch (e) {
      setStatus({ type: 'err', msg: lang === 'en' ? 'Network error' : 'Ikosa ryo kuri network' });
    }
  ...
  const personalInfo = [
    ...
    { icon: <LuIdCard size={20} />, key: lang === 'en' ? 'Officer ID' : 'ID', val: user.id || user.officer_id, color: '#8b5cf6' },
    { icon: <LuShield size={20} />, key: lang === 'en' ? 'Role' : 'Uruhare', val: roleLabel, color: '#0891b2' },
    ...(isSector ? [{ icon: <LuMapPin size={20} />, key: lang === 'en' ? 'Sector' : 'Umurenge', val: user.sector || 'Gashora', color: '#059669' }] : []),
    { icon: <LuBuilding2 size={20} />, key: lang === 'en' ? 'Department' : 'Ishami', val: user.department || 'Agriculture', color: '#0891b2' },
  ];
