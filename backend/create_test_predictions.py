#!/usr/bin/env python3
"""
Create Test Predictions for All Grade Levels
============================================

This script creates comprehensive test predictions covering all yield grade levels
(Below Average, Average, Good, Excellent) for farmers F001 and F002.
"""
...
# Test scenarios designed to produce different yield grades
TEST_SCENARIOS = [
    # BELOW AVERAGE scenarios (poor inputs)
    {
        "name": "Poor Maize - Local seed, no fertilizer",
        "farmer_id": "F001",
        "crop": "Maize",
        "season": "Season B",
        "farm_size": 50,
        ...
        "irrigation_used": "No",
        "pest_pressure": "High",
        "previous_crop": "Maize",  # Same crop reduces yield
        "terrain": "Valley",
        "expected_grade": "Below Average"
    },
    {
        "name": "Poor Beans - High pest, no irrigation",
        "farmer_id": "F002", 
        "crop": "Beans",
        "season": "Season B",
        "farm_size": 30,
    ...
    # AVERAGE scenarios (typical farmer inputs)
    {
        "name": "Average Maize - Improved seed, some fertilizer",
        "farmer_id": "F001",
        "crop": "Maize", 
        "season": "Season A",
        "farm_size": 100,
...
def create_test_predictions():
    ...
    print("🌾 Creating Test Predictions for All Grade Levels")
    print("=" * 60)
    ...
    for i, scenario in enumerate(TEST_SCENARIOS, 1):
        ...
        planting_date = (base_date + timedelta(days=i*3)).strftime('%Y-%m-%d')
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"   Yield: {yield_val} kg/are")
    ...
    farmer_counts = {}
    for result in results:
        farmer = result['farmer_id'] 
        farmer_counts[farmer] = farmer_counts.get(farmer, 0) + 1
    
    for farmer, count in sorted(farmer_counts.items()):
        print(f"   {farmer}: {count} predictions")
    ...
    for result in results:
        ...
        print(f"{status_icon} {result['prediction_id']} | {result['farmer_id']} | {result['crop']} | {result['yield']:.1f} kg/are | {result['actual_grade']}")
    
    return results

if __name__ == "__main__":
    results = create_test_predictions()
    print(f"\n🎉 Test data creation complete!")
    print(f"You can now test the frontend with farmers F001 and F002")
    print(f"Each farmer should have predictions across all grade levels.")